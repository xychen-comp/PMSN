...
class TEBN(nn.Module):
    ...
    def forward(self, input):
        y = input.permute(1,2,0,3,4).contiguous()  # T N C H W ,  N C T H W
        y = self.bn(y)
        y = y.contiguous().transpose(1, 2)
        y = y.transpose(0, 1).contiguous()  # NTCHW  TNCHW
        y = y * self.p.to(input.dtype)
        return y
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.sn1 = spiking_neuron(d_model=planes,**deepcopy(kwargs))
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = norm_layer(planes)
        self.sn2 = spiking_neuron(d_model=planes,**deepcopy(kwargs))
        self.downsample = downsample
        if downsample is not None:
            self.downsample_sn = spiking_neuron(d_model=self.downsample[0].out_channels,**deepcopy(kwargs))
        self.stride = stride
        self.cnf = cnf
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.sn1 = spiking_neuron(d_model=width,**deepcopy(kwargs))
        self.conv2 = conv3x3(width, width, stride, groups, dilation)
        self.bn2 = norm_layer(width)
        self.sn2 = spiking_neuron(d_model=width,**deepcopy(kwargs))
        self.conv3 = conv1x1(width, planes * self.expansion)
        self.bn3 = norm_layer(planes * self.expansion)
        self.sn3 = spiking_neuron(d_model=planes * self.expansion,**deepcopy(kwargs))
        self.downsample = downsample
        if downsample is not None:
            self.downsample_sn = spiking_neuron(inplanes,**deepcopy(kwargs))
        self.stride = stride
        self.cnf = cnf
...
class SEWResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.sn1 = spiking_neuron(d_model=self.inplanes,**deepcopy(kwargs))
